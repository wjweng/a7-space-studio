...
test('draft placement follows the pointer across interior conflicts and only rejects the exterior',()=>{
...
test('lights and ceiling beams remain editable drafts when their ceiling footprints overlap',()=>{
 const beam={id:'beam',name:'樑',type:'beam',x:2,z:2,w:1,d:.2,h:.2,rot:0},light={id:'light',name:'燈',type:'light',x:1,z:2,w:.3,d:.3,h:.1,rot:0};
 const result=placeAtTarget(light,{x:2,z:2},[light,beam]);assert.equal(result.blocked,false);assert(issues({...light,x:2,z:2},[light,beam]).length>0);
});
