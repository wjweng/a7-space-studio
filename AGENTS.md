...
- Door leaves turn about their hinge-side corner on the swing-side wall face (`doorLeaf` in `spatial.js`), not about the middle of their thickness; a mid-thickness pivot makes the hinge edge sweep through the jamb and look as if the hinge slides. Leaves fill the clear span between jambs; several openings (front door, bath A) start at a perpendicular wall, so `doorLeaf` probes `wallRects()` for the real jamb face instead of trusting the opening's ends. `doorRects` (collision and swing clearance) is a separate, older approximation and was deliberately left unchanged.
...
- A light whose whole footprint lies under a non-draft beam is mounted on it (`mountedOn`/`lightMountDrop` in `model.js`): it is not a clash, and `refreshValidity` drops its group by the beam depth (lifts it above the beam in top view). Anything that moves a beam must still end in `refreshValidity` so mounted lights follow.
