...
// Flush ceiling and linear lights are one downward-facing area light over the diffuser, so
// they spread evenly instead of leaving a bulb-like hot spot on the ceiling. Same total power as a point light of
// equal setting: an area light radiates pi*L*A, a point light 4*pi*I.
export const areaLightIntensity=(pointIntensity,area)=>4*pointIntensity/Math.max(area,1e-4);
...
export class SpaceScene{
 ...
 this.hemi=new T.HemisphereLight(0xf1f5ff,0x8c7965,2.2);this.scene.add(this.hemi);
 ...
 cyl(parent,r1,r2,h,x,y,z,mat){if(mat==='wood'&&this.woodOverride)mat=this.woodOverride;let m=new T.Mesh(new T.CylinderGeometry(r1,r2,h,32),typeof mat==='string'?this.m[mat]:mat);m.position.set(x,y,z);m.castShadow=true;m.receiveShadow=true;parent.add(m);return m;}
 ...
 for(const board of floorBoardRects())this.box(this.building,board.w,.009,board.d,board.x,.002,board.z,'floor');
 ...
 for(const door of doors){const g=new T.Group;g.position.set(door.x,0,door.z);g.rotation.y=door.angle;this.building.add(g);const pivot=new T.Group;g.add(pivot);pivot.position.set(visualDoorInset(door),0,-door.swing*.055);const width=visualLeafWidth(door);this.box(pivot,width,door.height,.045,width/2,door.height/2,0,'wood',.009);for(const side of[-1,1])this.box(pivot,.105,.025,.07,width-.12,1.02,side*.055,'metal',.01);pivot.userData.action=door.id;this.actions.set(door.id,{type:'door',pivot,base:0,def:door,amount:this.openStates[door.id]||0});for(const x of[0,door.width])this.box(g,.055,2.15,WALL_THICKNESS,x,1.075,0,'white');this.box(g,door.width+.1,.06,WALL_THICKNESS,door.width/2,2.13,0,'white');}
 this.curtains=[];for(const curtain of curtains){const{id,x,z,w}=curtain,g=new T.Group;g.position.set(x,0,z);g.userData.action=id;this.building.add(g);this.box(g,w,.05,.07,0,2.58,0,'dark');const panels=[];for(const sign of[-1,1]){let pg=new T.Group;g.add(pg);for(let j=0;j<16;j++){let fold=this.box(pg,w/32,2.38,.045,(j+.5)*w/32,1.36,Math.sin(j*Math.PI/2)*.04,'fabric',.015);fold.castShadow=false;}panels.push({g:pg,sign});}this.actions.set(id,{type:'curtain',panels,width:w,amount:this.openStates[id]||0,group:g});this.curtains.push(g);}
 ...
 makeFurniture(g,f){this.woodOverride=f.finish?this.finishMaterial(f.finish):null;try{return this.makeFurnitureParts(g,f);}finally{this.woodOverride=null;}}
 ...
 updateLight(){this.hemi.intensity=this.night?.28:1.75;if(this.fill)this.fill.intensity=this.night?.1:1.05;this.sun.intensity=this.night?.08:3.1;this.scene.background.set(this.night?'#77818a':'#dce4e2');for(const {f,point,area,span=Math.max(f.w,f.d)}of this.lightObjects||[]){const light=normalizeLight(f),spread=light.lightKind==='linear'?1:Math.max(.75,Math.min(1.6,Math.sqrt(Math.max(.01,f.w*f.d)/.0576))),base=(light.lumens/1200)*spread,color={white:'#eef6ff',natural:'#fff0cf',warm:'#ffc26f'}[light.colorTemperature];point.color?.set(color);point.distance=Math.max(2.8,5.5+span*2);const intensity=this.lightsOn&&light.on?(base*(light.dimming/100))*(this.night?1.2:.32):0;point.intensity=area?areaLightIntensity(intensity,area):intensity;}if(this.bounce)this.bounce.intensity=this.night?ceilingBounce(this.lightsOn?(this.lightObjects||[]).map(({f})=>normalizeLight(f)).filter(light=>light.on):[]):0;}
 ...
 canvas.addEventListener('pointermove',e=>{
  ...
  if(this.resizeDrag&&this.down.moved){let p=this.ground(e);if(p){this.resizeDrag.moved=true;this.onResize?.(this.resizeDrag.id,resizeAtHandle(this.resizeDrag.start,this.resizeDrag.axis,this.resizeDrag.sign,p).item);}}
