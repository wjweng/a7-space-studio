...
export class SpaceScene{
 ...
 const room=this.lightRoom?.();for(const {f,point}of this.lightObjects||[]){if(!point||point.userData?.fullIntensity===undefined)continue;const want=!room||point.castShadow||roomAt(f.x,f.z)===room?point.userData.fullIntensity:0;if(point.intensity!==want){point.intensity=want;changed=true;}}if(changed)this.shadowsDirty=true;this.shadowFocus=focus?{x:focus.x,z:focus.z}:null;}
...
updateRoomLight(){
 ...
 if(this.bounce&&this.lightRoom()!==this.litRoom){this.assignLampShadows();this.updateRoomLight();}
 const dirty=this.shadowsDirty||moving,cameraMoved=this.cameraChanged();
 if(!dirty&&!cameraMoved&&!controlsMoved&&performance.now()>(this.renderUntil||0))return;
