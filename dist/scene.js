...
export const coveFill=coves=>Math.min(1,.14*coves.reduce((sum,cove)=>sum+cove.lumens/1000*cove.dimming/100,0));
...
export class SpaceScene{
 ...
 this.bounce=new T.HemisphereLight(0x000000,0xfff1e0,0);this.scene.add(this.bounce);
 // Light coves light the room from above: their strip lights the ceiling, which then
 // lights floors and tops softly. One shared light from above stands in for that.
 this.coveFill=new T.HemisphereLight(0xffc26f,0x000000,0);this.scene.add(this.coveFill);this.sun=new T.DirectionalLight(0xffeed8,3.1);this.sun.position.set(-3,10,-5);this.sun.castShadow=false;this.sun.shadow.mapSize.set(2048,2048);Object.assign(this.sun.shadow.camera,{left:-12,right:12,top:12,bottom:-12,near:.5,far:40});this.sun.shadow.bias=-.0004;this.sun.shadow.normalBias=.025;this.scene.add(this.sun);
 ...
 this.makeMaterials();this.buildHouse();this.buildSurroundings();this.bind();this.resize();new ResizeObserver(()=>{this.resize();this.requestRender(100);}).observe(host);for(const type of['keydown','keyup','change','input'])addEventListener(type,()=>this.requestRender(400),{capture:true,passive:true});addEventListener('click',e=>{if(e.target.closest?.('button,input,select,label,a'))this.requestRender(400);},{capture:true,passive:true});for(const type of['pointerdown','pointerup','wheel'])host.addEventListener(type,()=>this.requestRender(400),{capture:true,passive:true});host.addEventListener('pointermove',()=>this.requestRender(150),{passive:true});this.renderer.setAnimationLoop(()=>this.frame());}
 ...
 // Default wet-room tiles reach the room boundaries on the wall centre lines,
 // so they run through the doorways instead of stopping short of them.
 for(const [x,z,w,d,room]of[[2.24,6,1.68,2.5,'衛浴 A'],[7.6675,3.55,2.195,1.7,'衛浴 B'],[7.415,5.79,1.69,2.78,'工作陽台']]){if(finished(room))continue;this.box(this.building,w,.018,d,x,.018,z,'tile');for(let xx=x-w/2;xx<x+w/2;xx+=.4)this.box(this.building,.004,.002,d,xx,.029,z,'white');for(let zz=z-d/2;zz<z+d/2;zz+=.4)this.box(this.building,w,.002,.004,x,.029,zz,'white');}
 ...
 assignLampShadows(){const focus=this.viewFocus?.(),budget=this.shadowBudget(),lamps=(this.lightObjects||[]).map(({point})=>point).filter(point=>point?.visible);const at=new T.Vector3,dist=point=>focus?Math.hypot(point.getWorldPosition(at).x-focus.x,at.z-focus.z):0;const chosen=new Set(lamps.map(point=>({point,d:dist(point)})).sort((a,b)=>a.d-b.d).slice(0,budget).map(({point})=>point));let changed=false;for(const {point}of this.lightObjects||[]){if(!point)continue;const cast=chosen.has(point);if(point.castShadow!==cast){point.castShadow=cast;changed=true;}}
 // Walls only stop light that casts a shadow. In walk view a lamp in another
 // room without a shadow would shine through them, so it is dimmed to zero.
 const room=this.lightRoom?.();for(const {f,point}of this.lightObjects||[]){if(!point||point.userData?.fullIntensity===undefined)continue;const want=!room||point.castShadow||roomAt(f.x,f.z)===room?point.userData.fullIntensity:0,now=this.fading?.has(point)?point.userData.fadeTarget:point.intensity;if(now!==want){this.setLightLevel(point,want);changed=true;}}if(changed)this.shadowsDirty=true;this.shadowFocus=focus?{x:focus.x,z:focus.z}:null;}
 ...
 fillTexture(material,kind,code,setup){
  const apply=({pixels,width,height})=>{
   ...
   setup?.(texture);
   material.map=texture;material.color.set(0xffffff);material.needsUpdate=true;this.requestRender?.();
  };
 ...
 if(type==='light'){const light=normalizeLight(f),thickness=Math.max(.02,Math.min(.3,h)),mountY=HEIGHT-thickness/2,round=light.shape==='round',size=Math.max(.08,Math.min(w,d)),temperature={white:['lightWhite','#eef6ff'],natural:['lightNatural','#fff0cf'],warm:['lightWarm','#ffc26f']}[light.colorTemperature],lightMat=fixtureLens(this.m[temperature[0]],temperature[1]);if(light.lightKind==='linear'){const depth=Math.max(.005,Math.min(.3,h));box(w,depth,d,0,HEIGHT-depth/2,0,lightMat);this.topOutline(box(w+.024,.004,d+.024,0,HEIGHT-.003,0,'dark'));const hitMaterial=new T.MeshBasicMaterial();hitMaterial.colorWrite=false;hitMaterial.depthWrite=false;hitMaterial.depthTest=false;const hit=new T.Mesh(new T.BoxGeometry(w,.02,Math.max(d,.2)),hitMaterial);hit.position.y=HEIGHT-depth/2;hit.castShadow=false;hit.receiveShadow=false;g.add(hit);if(!this.lightObjects)this.lightObjects=[];const count=linearLightCount(w);for(let i=0;i<count;i++){const x=-w/2+w*(i+.5)/count;this.lightObjects.push({f,lens:lightMat,point:this.downlight(g,x,HEIGHT-depth-.005,temperature[1]),share:1/count,span:w/count});}return;}if(light.lightKind==='pendant'){const drop=Math.max(.05,Math.min(HEIGHT-.12,light.pendantLength)),lampY=HEIGHT-drop;this.cyl(g,Math.max(.04,size*.20),Math.max(.04,size*.20),.025,0,HEIGHT-.013,0,'white');box(.014,Math.max(.03,drop-thickness*.8),.014,0,HEIGHT-(drop-thickness*.8)/2,0,'metal');if(round){this.cyl(g,size*.46,size*.34,thickness*1.45,0,lampY+thickness*.18,0,'white');this.topOutline(this.cyl(g,size*.46+.012,size*.46+.012,.004,0,lampY+thickness*.18+thickness*.725-.003,0,'dark'));this.cyl(g,size*.40,size*.40,thickness*.62,0,lampY-thickness*.28,0,lightMat);}else{box(w+.035,thickness*1.25,d+.035,0,lampY+thickness*.12,0,'white',Math.min(.05,w/5,d/5));this.topOutline(box(w+.059,.004,d+.059,0,lampY+thickness*.12+thickness*.625-.003,0,'dark'));box(Math.max(.04,w-.045),thickness*.55,Math.max(.04,d-.045),0,lampY-thickness*.28,0,lightMat,Math.min(.04,w/5,d/5));}}else if(round){this.cyl(g,size*.52,size*.52,thickness*.38,0,HEIGHT-thickness*.18,0,'white');this.topOutline(this.cyl(g,size*.52+.012,size*.52+.012,.004,0,HEIGHT-.003,0,'dark'));this.cyl(g,size*.46,size*.46,thickness*.78,0,mountY-thickness*.08,0,lightMat);}else{box(w+.035,thickness*.5,d+.035,0,HEIGHT-thickness*.24,0,'white',Math.min(.05,w/5,d/5));this.topOutline(box(w+.059,.004,d+.059,0,HEIGHT-.003,0,'dark'));box(Math.max(.04,w-.045),thickness*.78,Math.max(.04,d-.045),0,mountY-thickness*.08,0,lightMat,Math.min(.04,w/5,d/5));}if(!this.lightObjects)this.lightObjects=[];if(light.lightKind==='ceiling'){this.lightObjects.push({f,lens:lightMat,point:this.downlight(g,0,HEIGHT-thickness-.005,temperature[1])});return;}const point=new T.PointLight(temperature[1],0,Math.max(2.8,5.5+size*2),2);point.position.set(0,Math.max(.1,HEIGHT-light.pendantLength)-.03,0);point.userData.baseY=point.position.y;lampShadow(point,256);g.add(point);this.lightObjects.push({f,lens:lightMat,point});return;}
 ...
 if(type==='cornerShelf'){
  ...
  quarter(.015,.03);
  // Both straight boards are the unit's back (one on the wall, one against
  // the neighbouring cabinet); both are cut at each shelf so every cell can
  // take its own back finish.
  for(const cell of cells){const low=cell.bottom,high=Math.min(cell.bottom+cell.h,top),back=finish(cellFinish(f,cell,'back'));if(high>low+1e-6){box(w,high-low,.012,0,(low+high)/2,-d/2+.006,back);box(.018,high-low,d,w/2-.009,(low+high)/2,0,back);}}
...
// In a doorway the camera is in no room; keep the last one so lights do not flash.
lightRoom(){return this.mode==='walk'&&this.camera?roomAt(this.camera.position.x,this.camera.position.z)||this.litRoom||null:null;}
updateRoomLight(){
 const room=this.lightRoom(),here=({f})=>!room||roomAt(f.x,f.z)===room;this.litRoom=room;this.requestRender?.();
 if(this.bounce)this.setLightLevel(this.bounce,this.night?ceilingBounce(this.lightsOn?[...new Set((this.lightObjects||[]).filter(o=>!o.cove&&here(o)).map(({f})=>f))].map(normalizeLight).filter(light=>light.on):[]):0);
 if(this.coveFill){const coves=this.lightsOn?(this.lightObjects||[]).filter(o=>o.cove&&here(o)).map(({f})=>normalizeCove(f)).filter(cove=>cove.on):[],weight=cove=>cove.lumens*cove.dimming,total=coves.reduce((sum,cove)=>sum+weight(cove),0),color=new T.Color(0,0,0);for(const cove of coves)color.add(new T.Color({white:'#eef6ff',natural:'#fff0cf',warm:'#ffc26f'}[cove.colorTemperature]).multiplyScalar(weight(cove)/total));if(total)this.coveFill.color.copy(color);this.setLightLevel(this.coveFill,coveFill(coves)*(this.night?1:.32));}
}
// Walking into another room (or the shadow set moving with the view) changes
// which lamps reach the eye. Done at once it reads as a jump in brightness,
// so those changes ease in over about half a second, like eyes adjusting;
// switching a lamp or the day/night mode still applies at once.
setLightLevel(light,value){if(!this.fadeLights){this.fading?.delete(light);light.intensity=value;return;}(light.userData??={}).fadeTarget=value;(this.fading??=new Set).add(light);}
stepLightFade(dt){if(!this.fading?.size)return false;for(const light of this.fading){const target=light.userData.fadeTarget;light.intensity=T.MathUtils.damp(light.intensity,target,5,dt);if(Math.abs(light.intensity-target)<.01*Math.max(1,target)){light.intensity=target;this.fading.delete(light);}}return true;}
 ...
 // Top view looks down, so it draws what hangs lower higher up: a cabinet
 // under a beam sits just above the ceiling, and a light rises above
 // everything it hangs from (beam, cabinet), so each stays clickable.
 // Top view lifts a mounted light's fixture above what it hangs from so it
 // can be picked; its lamp stays at the real height, or it would light the
 // tops of the beam and cabinet it was lifted over.
 keepLampAtRealHeight(f,g){const shift=g.position.y+lightMountDrop(f,this.items||[]);for(const o of this.lightObjects||[])if(o.f===f&&o.point?.userData.baseY!==undefined){o.point.position.y=o.point.userData.baseY-shift;if(o.point.target)o.point.target.position.y=-shift;}}
 // A thin dark rim just under a fixture's top edge, shown only in top view:
 // from above a pale fixture on a pale beam or cabinet would vanish, and
 // from below the rim would read as a dark halo on the ceiling.
 topOutline(mesh){mesh.userData.topOnly=true;mesh.castShadow=false;mesh.receiveShadow=false;mesh.visible=this.mode==='top';return mesh;}
 mountOffset(f){if(f.type==='hangingCabinet'){const items=this.items||[];return this.mode==='top'&&mountDrop(f,items)>0?HEIGHT+.01:hangingElevation(f,items);}const drop=lightMountDrop(f,this.items||[]);if(!drop)return 0;if(this.mode!=='top')return -drop;const light=normalizeLight(f);return drop+(light.lightKind==='pendant'?light.pendantLength:0)+Math.max(.02,f.h)+.02;}
 refreshValidity(){if(!this.invalidHelpers||!this.invalidMarkers)return;for(const f of this.items){const g=this.groups.get(f.id);if(!g)continue;const foreground=f.draft&&f.id===this.foregroundDraft;g.visible=(this.mode==='top'||!f.draft)&&!(f.type==='beam'&&this.mode==='orbit'&&this.cutaway);g.position.y=(this.mode==='top'&&foreground?HEIGHT+.12:0)+this.mountOffset(f);if(f.type==='light'){this.keepLampAtRealHeight(f,g);g.traverse(o=>{if(o.userData.topOnly)o.visible=this.mode==='top';});}g.traverse(o=>{if(o.isMesh)o.renderOrder=foreground?200:0;});let h=this.invalidHelpers.get(f.id),marker=this.invalidMarkers.get(f.id);if(f.draft&&!h){h=new T.BoxHelper(g,0xd91515);this.invalidHelpers.set(f.id,h);this.scene.add(h);}if(f.draft&&!marker){marker=this.makeInvalidMarker(f);this.invalidMarkers.set(f.id,marker);this.scene.add(marker);}if(h){h.update();h.visible=f.draft&&this.mode==='top'&&f.id!==this.selected;if(!f.draft){this.scene.remove(h);h.geometry.dispose();h.material.dispose();this.invalidHelpers.delete(f.id);}}if(marker){marker.position.set(f.x,this.mode==='top'?(foreground?HEIGHT*2+.2:HEIGHT+.08):0,f.z);marker.rotation.y=f.rot*Math.PI/180;marker.visible=f.draft&&this.mode==='top';marker.traverse(o=>{if(o.isMesh)o.renderOrder=foreground?301:300;});if(!f.draft){this.scene.remove(marker);marker.traverse(o=>{o.geometry?.dispose();o.material?.dispose();});this.invalidMarkers.delete(f.id);}}}if(this.foregroundDraft&&!this.items.some(f=>f.id===this.foregroundDraft&&f.draft))this.foregroundDraft=null;const selected=this.items.find(f=>f.id===this.selected);if(this.selection&&selected){this.selection.material.color.set(selected.draft?0xd91515:0xc67748);this.selection.visible=this.mode==='top'||!selected.draft;this.selection.update();}}
 bind(){
 ...
 // The flat is only drawn when something could look different: shadows and
 // many lights make a frame expensive, and drawing an unchanged picture every
 // frame kept the main thread and GPU busy, so dragging a window or clicking
 // in the editor waited behind it. requestRender(ms) keeps drawing that long.
 // Pointer presses count only over the 3D view and clicks only on controls:
 // grabbing a floating window must not start a round of full renders.
 ...
 const fading=this.stepLightFade(dt);
 const dirty=this.shadowsDirty||moving,cameraMoved=this.cameraChanged();
 if(!dirty&&!fading&&!cameraMoved&&!controlsMoved&&performance.now()>(this.renderUntil||0))return;
 const shadowMap=this.renderer.shadowMap;if(shadowMap&&dirty){shadowMap.needsUpdate=true;this.shadowsDirty=false;}
...
for(const name of['buildHouse','buildFurniture','moveItem','resizeItem','updateLight','setCutaway','setMode','refreshValidity','updateBeamVisibility','setForegroundDraft','highlight']){const method=SpaceScene.prototype[name];if(method)SpaceScene.prototype[name]=function(...args){const result=method.apply(this,args);this.shadowsDirty=true;this.requestRender?.();return result;};}
