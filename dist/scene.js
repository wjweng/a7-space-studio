...
export class SpaceScene{
 ...
 for(const door of doors){const g=new T.Group;g.position.set(door.x,0,door.z);g.rotation.y=door.angle;this.building.add(g);const pivot=new T.Group;g.add(pivot);pivot.position.set(visualDoorInset(door),0,-door.swing*.055);const width=visualLeafWidth(door);this.box(pivot,width,door.height,.045,width/2,door.height/2,0,door.id==='door-0'?'entryDoor':'wood',.009);for(const side of[-1,1])this.box(pivot,.105,.025,.07,width-.12,1.02,side*.055,'metal',.01);pivot.userData.action=door.id;this.actions.set(door.id,{type:'door',pivot,base:0,def:door,amount:this.openStates[door.id]||0});for(const x of[0,door.width])this.box(g,.055,2.15,WALL_THICKNESS,x,1.075,0,'white');this.box(g,door.width+.1,.06,WALL_THICKNESS,door.width/2,2.13,0,'white');}
