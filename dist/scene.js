...
export class SpaceScene{
 ...
 mountOffset(f){if(f.type==='hangingCabinet')return hangingElevation(f,this.items||[]);const drop=lightMountDrop(f,this.items||[]);if(!drop)return 0;if(this.mode!=='top')return -drop;const light=normalizeLight(f);return(light.lightKind==='pendant'?light.pendantLength:0)+Math.max(.02,f.h)+.01;}
