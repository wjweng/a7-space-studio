...
export class SpaceScene{
 ...
 updateLight(){this.hemi.intensity=this.night?.32:2.2;this.sun.intensity=this.night?.08:3.1;this.scene.background.set(this.night?'#77818a':'#dce4e2');for(const {f,point}of this.lightObjects||[]){const light=normalizeLight(f),spread=Math.max(.75,Math.min(1.6,Math.sqrt(Math.max(.01,f.w*f.d)/.0576))),base=(light.lumens/1200)*spread,color={white:'#eef6ff',natural:'#fff0cf',warm:'#ffc26f'}[light.colorTemperature];point.color?.set(color);point.distance=Math.max(2.8,5.5+Math.max(f.w,f.d)*2);point.intensity=this.lightsOn&&light.on?(base*(light.dimming/100))*(this.night?1.2:.32):0;}}
