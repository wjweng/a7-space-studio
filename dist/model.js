...
import {validateCabinetDesign,cabinetFinishParts,hostsNicheTv} from './cabinet-design.js';
import {validateMediaWall} from './media-wall.js';
import {anchorMediaWall} from './wall-anchor.js';
...
export const minimums={sofa:[1.1,.5,.45],bed:[.65,1.2,.25],chair:[.3,.3,.55],wardrobe:[.2,.2,.3],drawer:[.2,.2,.2],console:[.5,.2,.2],mediaWall:[1,.2,1.5],television:[.3,.04,.2],table:[.3,.25,.2],desk:[.4,.35,.4],kitchen:[1.2,.4,.7],fridge:[.4,.4,.9],washer:[.4,.4,.6],sink:[.3,.3,.5],toilet:[.3,.4,.5],plant:[.2,.2,.3],shower:[.6,.6,1.8],rug:[.2,.2,.005],light:[.1,.1,.02],beam:[.05,.05,.05]};
...
export const mountedOn=(light,beam)=>light?.type==='light'&&beam?.type==='beam'&&(()=>{const a=beam.rot*Math.PI/180,c=Math.cos(a),s=Math.sin(a),tol=1e-6;return corners(light).every(([x,z])=>{const dx=x-beam.x,dz=z-beam.z;return Math.abs(c*dx-s*dz)<=beam.w/2+tol&&Math.abs(s*dx+c*dz)<=beam.d/2+tol;});})();
export const lightMountDrop=(light,items)=>light?.type==='light'?Math.max(0,...items.filter(beam=>!beam.draft&&mountedOn(light,beam)).map(beam=>Math.min(beam.h,HEIGHT))):0;
const ceilingClash=(a,b)=>!mountedOn(a,b)&&!mountedOn(b,a)&&overlaps(a,b);
export function issues(f,items){if(['light','beam'].includes(f.type)){let messages=items.filter(other=>other.id!==f.id&&['light','beam'].includes(other.type)&&ceilingClash(f,other)).map(other=>'與'+other.name+'重疊');if(f.type==='light'&&wallRects().some(w=>signedDistance(f,w)<-EPS))messages.push('與牆體重疊');if(f.type==='beam'&&!insideShell(f))messages.push('超出戶型邊界');return messages;}if(f.type==='rug')return[];let messages=[];if(corners(f).some(([x,z])=>!insideOrOutline(x,z)))messages.push('超出戶型邊界');if(f.type!=='television'&&wallRects().some(w=>signedDistance(f,w)<-EPS))messages.push('與牆體重疊');for(const other of items)if(other.id!==f.id&&!['rug','light','beam'].includes(other.type)&&!hostsNicheTv(f,other)&&!hostsNicheTv(other,f)&&sameRoom(f,other)&&furnitureInterference(f,other))messages.push('與'+other.name+'重疊');return messages;}
export function validateFurniture(input){if(!Array.isArray(input)||input.length>150)throw Error('家具資料格式不正確');let ids=new Set;return input.map(f=>{if(!f||typeof f.id!=='string'||ids.has(f.id)||!minimums[f.type]||typeof f.name!=='string')throw Error('家具種類或編號不正確');ids.add(f.id);const limits=minimumsFor(f),absoluteMin=f.type==='beam'?.05:Math.min(.1,limits[1]);if(f.w<limits[0]||f.d<limits[1]||f.h<limits[2])throw Error('此家具最小寬／深／高為 '+limits.map(n=>Math.round(n*100)).join('／')+' cm');for(const k of['x','z','w','d','h','rot'])if(!Number.isFinite(f[k]))throw Error('尺寸必須為有效數字');if(f.w<absoluteMin||f.d<absoluteMin||f.h<.005||f.w>5||f.d>5||f.h>HEIGHT||Math.abs(f.x)>20||Math.abs(f.z)>20)throw Error('尺寸或位置超出允許範圍');const next={id:f.id,type:f.type,name:f.name.slice(0,60),x:f.x,z:f.z,w:f.w,d:f.d,h:f.h,rot:f.rot,open:f.open?1:0,doorStyle:['left','right','double','multi','drawers','mixed','sliding'].includes(f.doorStyle)?f.doorStyle:'double',draft:!!f.draft,assumed:true};if(finishableTypes.includes(f.type)&&finishByCode(f.finish))next.finish=f.finish;if(f.cabinetDesign&&f.partFinishes&&typeof f.partFinishes==='object'){const parts={};for(const [key]of cabinetFinishParts)if(finishByCode(f.partFinishes[key]))parts[key]=f.partFinishes[key];if(Object.keys(parts).length)next.partFinishes=parts;}if(fabricTypes.includes(f.type)&&normalizeFabric(f.fabric))next.fabric=normalizeFabric(f.fabric);if(f.type==='sink')next.basin=normalizeSinkBasin(f);if(f.type==='kitchen')next.kitchenParts=normalizeKitchenParts(f);if(f.type==='light')Object.assign(next,normalizeLight(f));if(f.type==='mediaWall'){next.mediaWall=validateMediaWall(f,f.mediaWall);if(f.wallAnchor){if(!Number.isInteger(f.wallAnchor.index)||!Number.isFinite(f.wallAnchor.offset)||![-1,1].includes(f.wallAnchor.side))throw Error('電視牆固定牆面不正確');const aligned=anchorMediaWall(next,f.wallAnchor,walls,WALL_THICKNESS);Object.assign(next,{x:aligned.x,z:aligned.z,rot:aligned.rot,wallAnchor:aligned.wallAnchor});}}if(f.type==='television'){if(!Number.isFinite(f.elevation)||f.elevation<0||f.elevation+f.h>HEIGHT)throw Error('電視高度超出空間');next.elevation=f.elevation;next.tvMount=['wall','cabinet','niche'].includes(f.tvMount)?f.tvMount:'wall';if(next.tvMount==='niche'&&(typeof f.supportId!=='string'||typeof f.supportCell!=='string'))next.tvMount='wall';if(next.tvMount!=='wall'&&typeof f.supportId==='string')next.supportId=f.supportId;if(next.tvMount==='niche')next.supportCell=f.supportCell;}if(['wardrobe','console'].includes(f.type)&&f.cabinetDesign){next.cabinetDesign=validateCabinetDesign(f,f.cabinetDesign);next.openCells={};for(const cell of next.cabinetDesign.columns.flatMap(column=>column.cells))if(cell.front!=='open'&&f.openCells?.[cell.id])next.openCells[cell.id]=1;}return next;});}
