...
export function issues(f,items){if(['light','beam'].includes(f.type))return items.filter(other=>other.id!==f.id&&['light','beam'].includes(other.type)&&overlaps(f,other)).map(other=>'與'+other.name+'重疊');if(f.type==='rug')return[];let messages=[];if(corners(f).some(([x,z])=>!insideOrOutline(x,z)))messages.push('超出戶型邊界');if(wallRects().some(w=>signedDistance(f,w)<-EPS))messages.push('與牆體重疊');for(const other of items)if(other.id!==f.id&&!['rug','light','beam'].includes(other.type)&&sameRoom(f,other)&&furnitureInterference(f,other))messages.push('與'+other.name+'重疊');return messages;}
